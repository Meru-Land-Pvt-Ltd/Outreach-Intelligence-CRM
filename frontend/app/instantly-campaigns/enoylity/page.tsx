...
export default async function EnoylityInstantlyPage() {
  const response = await apiGet("/instantly/enoylity");
  const rows = response?.data || [];

  return (
    <DataSection title="Enoylity Instantly" description={`${rows.length} records`}>
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Enoylity Related</TableHead>
        ...
        <TableBody>
          ...
          {rows.length === 0 ? (
            <TableEmpty
              colSpan={11}
              title="No Enoylity Instantly rows yet."
