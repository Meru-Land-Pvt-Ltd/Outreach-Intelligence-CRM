...
export default async function ProspeoRawPage() {
  const response = await apiGet("/email-discovery/prospeo-raw");
  const rows = response?.data || [];

  return (
    <main className="w-full space-y-6">
      <PageHeader title="Prospeo Raw Contacts" description="Raw Prospeo contacts." />
      <EmailDiscoveryTabs />

      <DataSection title="Prospeo Raw Contacts" description={`${rows.length} records`}>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            ...
            {rows.length === 0 ? (
              <TableEmpty
                colSpan={6}
                title="No Prospeo raw contacts yet."
                description="Prospeo contacts will appear after email discovery."
              />
            ) : null}
          </TableBody>
        </Table>
